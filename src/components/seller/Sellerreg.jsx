...
function Sellerreg() {
    ...
    useEffect(() => {
        ...
        const isSeller = localStorage.getItem('user');
        if (userData) {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            const response = await axios.post(`${import.meta.env.VITE_BACKEND_URL}seller/sellersignup`, formData, {
                headers: {
        ...
        return (
          <div className="min-h-screen bg-gray-200 p-4 flex items-center justify-center ">
            <div className="text-lg shadow-2xl py-6 bg-gray-100 text-gray-600 rounded-xl flex flex-col justify-center items-center w-md">
                ...
                <button onClick={() => nav('/')} className='py-1 px-2 cursor-pointer rounded-xl bg-gray-600 text-white mt-2'>back to home</button>
            </div>
          </div>
        );
      }
    ...
    return (
        <div className="min-h-screen bg-gray-50 py-8 px-4 flex items-center justify-center">
            <div className="w-xl mx-auto bg-black rounded-lg shadow-md p-6 relative">
                <ArrowLeftCircle size={30} color='white' className='absolute top-6  left-4 cursor-pointer hover:transform hover:scale-110 duration-200' onClick={() => nav('/')} />
                <h2 className="text-2xl text-center font-bold mb-6 text-gray-100">Become a Seller</h2>
